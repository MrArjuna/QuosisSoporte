...
netsh advfirewall firewall add rule name="saci	exe" dir=	in	action=allow program="%programfiles(x86)%\Compac\Servidor de Aplicaciones\saci.exe"	enable=yes profile=any	
netsh advfirewall firewall add rule name="saci admin	exe" dir=	in	action=allow program="%programfiles(x86)%\Compac\Servidor de Aplicaciones\SaciAdmin.exe"	enable=yes profile=any	
echo "EJECUTABLES SALIDA"
netsh advfirewall firewall add rule name="saci	exe" dir=	out	action=allow program="%programfiles(x86)%\Compac\Servidor de Aplicaciones\saci.exe"	enable=yes profile=any	
netsh advfirewall firewall add rule name="saci admin	exe" dir=	out	action=allow program="%programfiles(x86)%\Compac\Servidor de Aplicaciones\SaciAdmin.exe"	enable=yes profile=any	
